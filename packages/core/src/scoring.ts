import type { Listing, ScoreReasons, ScoringWeights, SearchProfile } from "./types";
...
export function scoreListing(
  ...
  const wt = { ...DEFAULT_WEIGHTS, ...weights };
  const wd = (base: number, category: keyof ScoringWeights): number =>
    Math.round(base * wt[category]);
  const rules: Rule[] = [];
  ...
  if (profile.price_max) {
    ...
    if (evaluable && listing.price != null) {
      if (listing.price <= profile.price_max) {
        ...
        let penalty: number;
        if (ratio < 0.1) penalty = wd(15, "price");
        else if (ratio < 0.25) penalty = wd(35, "price");
        else if (ratio < 0.5) penalty = wd(60, "price");
        else {
          penalty = 100;
          killed = true;
        }
  ...
  if (profile.rooms_min) {
    ...
    if (evaluable && listing.rooms != null) {
      if (listing.rooms >= profile.rooms_min) {
        ...
        rules.push({ rule: `Pièces insuffisantes (${listing.rooms} < ${profile.rooms_min})`, delta: -wd(15, "rooms"), positive: false });
      }
    }
  }
  ...
  if (profile.city) {
    ...
    if (evaluable && listing.city) {
      if (norm(listing.city).includes(norm(profile.city))) {
        ...
        rules.push({ rule: `Ville différente (${listing.city})`, delta: -wd(10, "location"), positive: false });
      }
    }
  }
  ...
  if (profile.property_type && profile.property_type !== "any") {
    ...
    if (evaluable && listing.property_type) {
      ...
      if (matchesPropertyType(profile.property_type, listing.property_type)) {
        ...
        rules.push({ rule: `Type de bien différent (recherche ${label})`, delta: -wd(30, "property_type"), positive: false });
      }
    }
  }
  ...
  if (profile.furnished && profile.furnished !== "any") {
    ...
    if (evaluable && listing.furnished !== null) {
      ...
      if (listing.furnished === want) {
        ...
        rules.push({ rule: `Type meublé non conforme`, delta: -wd(10, "furnished"), positive: false });
      }
    }
  }
  ...
  if (must.length > 0) {
    ...
    if (found.length > 0) {
      rules.push({ rule: `Mots-clés positifs trouvés (${found.join(", ")})`, delta: wd(5, "keywords") * found.length, positive: true });
    }
    const missing = must.filter((k) => !lower.includes(norm(k)));
    if (missing.length > 0) {
      rules.push({ rule: `Mots-clés positifs manquants (${missing.join(", ")})`, delta: -wd(5, "keywords") * missing.length, positive: false });
    }
  }
